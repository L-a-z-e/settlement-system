...
import com.laze.settlementsystem.domain.repository.CustomerRepository;
import com.laze.settlementsystem.support.EmailProvider;
...
public class SettleGroupItemMailWriter implements ItemWriter<List<SettleGroup>> {
    ...
    public void write(Chunk<? extends List<SettleGroup>> chunk) throws Exception {
        for (List<SettleGroup> settleGroups : chunk) {
            ...
            final Long totalCount = settleGroups.stream()
                    .map(SettleGroup::getTotalCount).reduce(0L, Long::sum);
            final Long totalFee = settleGroups.stream()
                    .map(SettleGroup::getTotalFee).reduce(0L, Long::sum);
